...
export const SearchHistoryList = () => {
  ...
  return (
    <div className={`pt-4 mt-6 rounded-[24px] p-4 ${historyCardBg}`}>
      <h2 className="text-lg flex items-center mb-4">Search History</h2>

      {searchHistory.length === 0 ? (
        <p className="text-sm italic">
          No search records yet for this session.
        </p>
      ) : (
        <ul className="space-y-2">
          {searchHistory.map((record) => (
            <li
              key={record.id}
              className={`flex justify-between items-center  rounded-lg text-sm p-4 ${searchItemBg}`}
            >
              {/* Content */}
              <div className="grow flex flex-col sm:flex-row justify-between mr-3">
                ...
                <p
                  ...
                  style={{ opacity: isDarkMode ? 0.5 : 0.7 }}
                >
